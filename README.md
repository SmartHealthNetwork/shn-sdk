...
| `BuildOrderSelectRequest` / `ParseCards` | CRD order-select request; `ParseCards` → a `CardCoverage` (`Covered`, `PANeeded`, `Questionnaires[]`, `SatisfiedPaID`) with `.PARequired()` / `.NeedsDTR()` helpers — the faithful-minimal projection of Da Vinci CRD coverage-information. |
