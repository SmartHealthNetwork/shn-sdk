...
| `BuildQuestionnaireFetch` / `BuildQuestionnaireFetchWithCoverage` / `ExtractQuestionnaireFromPackage` / `ParseQuestionnaireURL` | DTR questionnaire-fetch request; the response is a Da Vinci `$questionnaire-package` collection Bundle (Questionnaire + dependent Libraries/ValueSets) — `ExtractQuestionnaireFromPackage` returns the bare `Questionnaire`, then `ParseQuestionnaireURL` reads its canonical url. (`BuildQuestionnairePackage` is the responder-side wrapper.) `BuildQuestionnaireFetchWithCoverage(canonical, coverageJSON)` additionally carries your Coverage on the request — required to reach a `pa.dtr@2.2` responder, which derives its `QuestionnaireResponse` shell's coverage reference from that resource's `Coverage.id` and fails closed without one. The canonical-only `BuildQuestionnaireFetch` remains for 2.0-only callers. |
