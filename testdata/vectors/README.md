...
| `crd-cards-pa.json` | CONSUME | `shnsdk.ParseCRDResponse` → one order (`ServiceRequest/sr-MBR-COVERED`) whose coverage information says covered, `auth-needed`, questionnaire `http://smarthealth.network/fhir/Questionnaire/pa-lumbar-mri` — the PA-required CRD answer for 72148, built by `shnsdk.BuildCRDResponse` (coverage information as an update system action, no card). The deprecated `shnsdk.ParseCards` reads the same projection. |
| `questionnaire.json` | INPUT | The payer's demo lumbar-MRI DTR `Questionnaire` fed into `shnsdk.FillQuestionnaire`. The SDK can't build the payer fixture (it's substrate-only), so it ships as an input. |
...
| `claim-bundle.json` | **REPRODUCE** (historical) | Minted by the now-removed `BuildClaimBundle(qr, sr, "Patient/MBR-COVERED", "Coverage/MBR-COVERED", "golden-corr", clock)`, deleted with the minimized PA shape (`04ac7d81`). The current PAS submit builder is `shnsdk.BuildConformantClaimBundle(shnsdk.ConformantClaimInputs{QR: qr, SR: sr, PatientRef: "Patient/MBR-COVERED", CoverageRef: "Coverage/MBR-COVERED", MemberID: "MBR-COVERED", Corr: "golden-corr", Created: clock})` — it builds a structurally different conformant shape and does not reproduce this frozen fixture (see the per-line orphan note below). |
...
| `servicerequest-uc04.json` | INPUT | The pended→amend lumbar-MRI `ServiceRequest` (CPT 72148, patient `MBR-UC04`) fed into the now-removed `BuildClaimUpdateBundle` (current sibling: `shnsdk.BuildConformantClaimUpdateBundle`). |
| `questionnaireresponse-uc04.json` | INPUT | Pended→amend filled QR (prior-surgery context: `PriorSurgery=true`, 6 weeks conservative therapy) fed into the now-removed `BuildClaimUpdateBundle` (current sibling: `shnsdk.BuildConformantClaimUpdateBundle`). |
| `claimupdate-bundle-uc04.json` | **REPRODUCE** (historical) | Minted by the now-removed `BuildClaimUpdateBundle(qr, sr, dr, prov, "Patient/MBR-UC04", "Coverage/MBR-UC04", "golden-corr-update", "golden-corr", clock)`, deleted with the minimized PA shape. The current PAS update builder is `shnsdk.BuildConformantClaimUpdateBundle(shnsdk.ConformantClaimUpdateInputs{QR: qr, SR: sr, PatientRef: "Patient/MBR-UC04", CoverageRef: "Coverage/MBR-UC04", MemberID: "MBR-UC04", Provenance: prov, DiagnosticReport: dr, Corr: "golden-corr-update", OriginalCorr: "golden-corr", Created: clock})` — it builds a structurally different conformant shape and does not reproduce this frozen fixture (see the per-line orphan note below). |
